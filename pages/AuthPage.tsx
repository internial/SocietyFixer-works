...
/**
 * AuthPage handles user login and registration.
 ...
 * Password recovery functionality has been removed.
 */
export default function AuthPage() {
    const navigate = useNavigate();
    const location = useLocation();
    const searchParams = new URLSearchParams(location.search);
    const view = searchParams.get('view') || 'login'; // 'login' or 'register'
    ...
    // SEO
    const pageTitle = view === 'register' ? 'Create an Account' : 'Login';
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        if (view === 'login') {
            ...
            if (error) {
                ...
                addToast({ message: "Account created successfully! You are now logged in.", type: "success" });
                // On success, Supabase creates a session, so we can redirect immediately.
                navigate('/');
            }
        }
        setLoading(false);
